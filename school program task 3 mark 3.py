...
def get_menu_selection():
    ...
    while user_choice < 1 or user_choice > 5:   # whatever indented under it will run as long as user_choice is <1 or >5
        ...
        Press \'2\' to view in alphabetical order with each student’s highest score for the tests
        Press \'3\' to view scores by highest, highest to lowest
        Press \'4\' to view scores by average, highest to lowest
        Press \'5\' to exit
        """)    # outputs the text
...
# description: this function works out the average score of a player in the list
# parameters: the pupils first score, second score and third score
# return: the pupils average score
def get_avg_score(score_1, score_2, score_3):
    # Initialise local variables
    average_score = 0
    number_of_scores = 0

    if score_1 != "":   # this checks if score_1 equals ""  and if not, it will run whatever is indented under it
        average_score += int(score_1)  # updated variable to add score_1 as an integer
        number_of_scores += 1  # updated variable to add 1
    elif score_2 != "":   # this checks if score_2 equals ""  and if not, it will run whatever is indented under it
        average_score += int(score_2)  # updated variable to add score_2 as an integer
        number_of_scores += 1  # updated variable to add 1
    elif score_3 != "":   # this checks if score_3 equals ""  and if not, it will run whatever is indented under it
        average_score += int(score_3)  # updated variable to add score_3 as an integer
        number_of_scores += 1  # updated variable to add 1
    average_score /= number_of_scores   # this will davide the average score by the number of scores
    return average_score  # this will return the variable containing the average score
...
while choice != 5:     # whatever indented under it will run as long as choice is not equal to 5
    ...
    if choice == 1:
        ...
        # get average score for each pupil and sort the list using it (descending)
        pupil_data.sort(key=lambda field: get_avg_score(field[1], field[2], field[3]), reverse=True)
        print("\n\nScores by average, highest to lowest from class", pupil_class)   # this outputs text and variable
        print("\n" + '%-30s' % "Players Name" + " Score")   # this outputs text
        print('-' * 36)     # this underlines the headings above
        for pupil in pupil_data:    # this creates a for loop
            avg_score = get_avg_score(pupil[1], pupil[2], pupil[3])  # this compares all three scores to find an average
            print('%-30s' % pupil[0] + " " + str(avg_score))  # outputs the variable containing the name & average score
